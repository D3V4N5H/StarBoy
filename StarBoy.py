...
'''
paradigmatic_Query_Response_Dictionary = {'jaccard':[]}
...
def paradigmatic_Query_Response_Function(tx):
	for record in tx.run(paradigmatic_Query_Response):
		...
		#list_Of_Tracks[song_Name]
		paradigmatic_Query_Response_Dictionary["jaccard"].append({tuple([record["s.word"], record["o.word"]]): record["r.paradig"]})
